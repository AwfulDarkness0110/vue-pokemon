...
const filtered = computed(
  () => result.value?.pokemon_v2_pokemon.filter(
    (p: Pokemon_V2_Pokemon) => favorites.value.has(p.id)
  )
)
const pokemons = computed(() => onlyFavorites.value ? filtered.value : result.value?.pokemon_v2_pokemon)
const total = computed(() => pokemons.value?.length)

const subtitle = computed(
  () => 0 + total.value > 0
    ? `${total.value} pokemons in ${onlyFavorites.value ? 'favorites' : 'the list'}`
    : 'No pokemons in this section yet!'
)
</script>

<template>
  <v-bottom-navigation active v-model="currentItem" grow horizontal :elevation="3">
    <v-btn variant="text" v-for="item in items" :key="item.text">
  ...
  <pokemon-list :pokemons="pokemons">
    <v-btn v-if="!onlyFavorites" class="mt-8" @click="loadMore" :loading="loading">Load More</v-btn>
  </pokemon-list>
</template>
